...
RUN cat /etc/security/limits.conf | grep -v oracle | tee /etc/security/limits.conf && \
    cd /root && echo 'Unzipping' && unzip -q p10404530_112030_Linux-x86-64_1of7.zip && \
    unzip -q p10404530_112030_Linux-x86-64_2of7.zip && rm -f p10404530_112030_Linux*.zip && mv database /home/oracle/ && \
